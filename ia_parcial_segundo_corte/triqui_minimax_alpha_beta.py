...
from utilities.State import AlphaBetaState as State
...
""" Genera los estados mínimos de cada posible tablero según el algoritmo de minimax ignorando(Podando) los estados cuyo padre sea <= beta(valor actual) """


def generate_min_states(current_state, letter, opponent_letter):
    value = get_value(current_state)
    current_state.value = math.inf
    # keep Searching
...
""" Genera los estados máximos de cada posible tablero según el algoritmo de minimax ignorando(Podando) los estados cuyo padre sea >= alpha(valor actual) """
        ...
        for row_index in range(0, len(board)):
            for column_index in range(0, len(board[0])):
                if(board[row_index][column_index] == None):
                    ...
                    new_state = State(new_board, parent=current_state)
                    current_state.addChild(new_state)
                    child_value = generate_min_states(
                        new_state,  opponent_letter, letter)
